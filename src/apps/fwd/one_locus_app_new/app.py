...
def create_pop_size_panel(pop_idx, pop_config):
    size_slider = ui.input_slider(
        id=f"pop_{pop_idx}_size_slider",
        label="",
...
def create_freqs_panel(pop_idx, pop_config):
    ...
    freq_a_slider = ui.input_slider(
        id=f"pop_{pop_idx}_freq_a_slider",
        label="Freq. A",
        min=0,
        max=1,
        value=freq_A,
    ...
    geno_freqs_slider = ui.input_slider(
        id=f"pop_{pop_idx}_geno_freqs_slider",
    ...
    genotypic_freqs = ui.layout_columns(
        ui.value_box(
            "Freq. AA",
            freq_AA,
            id=f"pop_{pop_idx}_freq_AA_output",
        ),
        ui.value_box(
            "Freq. Aa",
            freq_Aa,
...
def create_selfing_rate_panel(pop_idx, pop_config):
    slider = ui.input_slider(
        id=f"pop_{pop_idx}_selfing_slider",
        label="",
        min=0,
        max=1,
...
def create_selection_panel(pop_idx, pop_config):
    fitness = pop_config["fitness"]
    wAA_slider = ui.input_slider(
        id=f"pop_{pop_idx}_wAA_slider",
        label="wAA",
        min=0,
        max=1,
        value=fitness["wAA"],
...
def create_mutation_panel(pop_idx, pop_config):
    mutation = pop_config["mutation"]
    A2a_slider = ui.input_slider(
        id=f"pop_{pop_idx}_A2a_slider",
        label="A2a rate",
        min=0,
...
def create_pop_accordion(pop_idx, pop_config):
    freqs_panel = create_freqs_panel(pop_idx, pop_config)
    size_panel = create_pop_size_panel(pop_idx, pop_config)
    panels = [freqs_panel, size_panel]
    if "fitness" in pop_config:
        selection_panel = create_selection_panel(pop_idx, pop_config)
        panels.append(selection_panel)
    if "mutation" in pop_config:
        mutation_panel = create_mutation_panel(pop_idx, pop_config)
        panels.append(mutation_panel)
    if "selfing_rate" in pop_config:
        selfing_rate_panel = create_selfing_rate_panel(pop_idx, pop_config)
        panels.append(selfing_rate_panel)

    pop_accordion = ui.accordion(
        *panels,
        id=f"pop_{pop_idx}_accordion",
...
def create_pops_panel(config):
    ...
    for pop_idx in sorted(config["pops"].keys()):
        ...
        pop_accordion = create_pop_accordion(pop_idx, pop_config)
